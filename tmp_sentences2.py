...
with open('tmp_sentences_output2.txt', 'w', encoding='utf-8') as out:
    for fn in sorted([f for f in glob.glob('*.html') if f not in ('index.html','page.html','tmp_extract.py','tmp_keyword_extract.py','tmp_keyword_extract2.py','tmp_search.py','tmp_sentences.py','tmp_sentences2.py')]):
        out.write(f'FILE: {fn}\n')
